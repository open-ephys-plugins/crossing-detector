...
CrossingDetectorSettings::CrossingDetectorSettings() : inputChannel (0),
                                                       eventChannel (0),
                                                       thresholdChannel (0),
                                                       sampleRate (0.0f),
                                                       ...
                                                       turnoffEvent (nullptr)
{
    ...
    eventMetadataDescriptors.add (new MetadataDescriptor (MetadataDescriptor::INT64, 1, "Crossing Point", "Time when threshold was crossed", "crossing.point"));
    eventMetadataDescriptors.add (new MetadataDescriptor (MetadataDescriptor::FLOAT, 1, "Crossing level", "Voltage level at first sample after crossing", "crossing.level"));
    eventMetadataDescriptors.add (new MetadataDescriptor (MetadataDescriptor::FLOAT, 1, "Threshold", "Monitored voltage threshold", "crossing.threshold"));
    eventMetadataDescriptors.add (new MetadataDescriptor (MetadataDescriptor::UINT8, 1, "Direction", "Direction of crossing: 1 = rising, 0 = falling", "crossing.direction"));
}
...
TTLEventPtr CrossingDetectorSettings::createEvent (juce::int64 bufferTs, int crossingOffset, int bufferLength, float threshold, float crossingLevel, bool eventState)
{
    ...
    if (eventState)
    {
        ...
        TTLEventPtr eventOn = TTLEvent::createTTLEvent (eventChannelPtr, eventTsOn, eventChannel, true, mdArray);
    ...
    else
    {
        ...
        TTLEventPtr eventOff = TTLEvent::createTTLEvent (eventChannelPtr, eventTsOff, eventChannel, false, mdArray);
...
CrossingDetector::CrossingDetector()
    : GenericProcessor ("Crossing Detector"), thresholdType (CONSTANT), constantThresh (0.0f), selectedStreamId (0), posOn (true), negOn (false), eventDuration (100), timeout (1000), useBufferEndMask (false), bufferEndMaskMs (3), pastStrict (1.0f), pastSpan (0), futureStrict (1.0f), futureSpan (0), useJumpLimit (false), jumpLimit (5.0f), jumpLimitElapsed (0), sampToReenable (pastSpan + futureSpan + 1), pastSamplesAbove (0), futureSamplesAbove (0), inputHistory (pastSpan + futureSpan + 2), thresholdHistory (pastSpan + futureSpan + 2)
{
...
void CrossingDetector::registerParameters()
{
    ...
    addBooleanParameter (Parameter::PROCESSOR_SCOPE, "rising", "Rising", "Trigger events when past samples are below and future samples are above the threshold", posOn);

    addBooleanParameter (Parameter::PROCESSOR_SCOPE, "falling", "Falling", "Trigger events when past samples are above and future samples are below the threshold", negOn);

    addIntParameter (Parameter::PROCESSOR_SCOPE, "timeout", "Timeout (ms)", "Minimum length of time between consecutive events", timeout, 0, 10000);

    addIntParameter (Parameter::PROCESSOR_SCOPE, "threshold_type", "Threshold Type", "Type of Threshold to use", thresholdType, 0, 2);

    addFloatParameter (Parameter::PROCESSOR_SCOPE, "constant_threshold", "Const Threshold", "Constant threshold value", "mV", constantThresh, -FLT_MAX, FLT_MAX, 0.1f);

    addFloatParameter (Parameter::PROCESSOR_SCOPE, "min_random_threshold", "Min Random", "Minimum random threshold value", "mV", randomThreshRange[0], -10000.0f, 10000.0f, 0.1f);

    addFloatParameter (Parameter::PROCESSOR_SCOPE, "max_random_threshold", "Max Random", "Maximum random threshold value", "mV", randomThreshRange[1], -10000.0f, 10000.0f, 0.1f);

    addIntParameter (Parameter::STREAM_SCOPE, "threshold_chan", "Threshold Channel", "Threshold reference channel", 0, 0, 1000);

    addIntParameter (Parameter::PROCESSOR_SCOPE, "past_span", "Past Span", "Number of past samples to look at at each timepoint (attention span)", pastSpan, 0, 100000);

    addIntParameter (Parameter::PROCESSOR_SCOPE, "future_span", "Future Span", "Number of future samples to look at at each timepoint (attention span)", futureSpan, 0, 100000);

    addIntParameter (Parameter::PROCESSOR_SCOPE, "past_strict", "Past Fraction", "Percentage of past span required to be above / below threshold", pastStrict, 1, 100);

    addIntParameter (Parameter::PROCESSOR_SCOPE, "future_strict", "Future Fraction", "Percentage of future span required to be above / below threshold", futureStrict, 1, 100);

    addBooleanParameter (Parameter::PROCESSOR_SCOPE, "use_jump_limit", "Use Jump Limit", "Enable/Disable phase jump filtering", useJumpLimit);

    addFloatParameter (Parameter::PROCESSOR_SCOPE, "jump_limit", "Jump Limit", "Maximum jump size", "mV", jumpLimit, 0.0f, FLT_MAX, 0.1f);

    addFloatParameter (Parameter::PROCESSOR_SCOPE, "jump_limit_sleep", "", "Sleep after artifact", "ms", 0.0f, 0.0f, FLT_MAX, 0.1f);

    addBooleanParameter (Parameter::PROCESSOR_SCOPE, "use_buffer_end_mask", "Use Buffer End Mask", "Enable/disable buffer end sample voting", negOn);

    addIntParameter (Parameter::PROCESSOR_SCOPE, "buffer_end_mask", "Buffer End Mask", "Ignore crossings ocurring specified ms before the end of a buffer", bufferEndMaskMs, 0, INT_MAX);
...
void CrossingDetector::process (AudioSampleBuffer& continuousBuffer)
{
    ...
    for (auto stream : getDataStreams())
    {
        if ((*stream)["enable_stream"] && stream->getStreamId() == selectedStreamId)
        {
            ...
            int turnoffOffset = settingsModule->turnoffEvent ? jmax (0, (int) (settingsModule->turnoffEvent->getSampleNumber() - startTs)) : -1;
            ...
            for (int i = 0; i < nSamples; ++i)
            {
                ...
                if (indCross < sampToReenable || (useBufferEndMask && nSamples - indCross > settingsModule->bufferEndMaskSamp))
                {
                ...
                if ((currPosOn && shouldTrigger (true, preVal, postVal, preThresh, postThresh)) || (currNegOn && shouldTrigger (false, preVal, postVal, preThresh, postThresh)))
                {
                    // create and add ON event
                    TTLEventPtr onEvent = settingsModule->createEvent (startTs, indCross, nSamples, postThresh, postVal, true);
                    addEvent (onEvent, std::max (indCross, 0));

                    // create OFF event
                    int sampleNumOff = std::max (indCross, 0) + settingsModule->eventDurationSamp;
                    TTLEventPtr offEvent = settingsModule->createEvent (startTs, indCross, nSamples, postThresh, postVal, false);
...
bool CrossingDetector::shouldTrigger (bool direction, float preVal, float postVal, float preThresh, float postThresh)
{
